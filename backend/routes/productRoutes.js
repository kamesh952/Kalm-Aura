...
router.get("/", async (req, res) => {
  try {
    ...
    if (gender && gender.toLowerCase() !== "all") query.gender = gender;
...
router.get("/similar/:id", async (req, res) => {
  ...
  try {
    ...
    console.error("Error in /similar/:id:", error.stack);
    res.status(500).json({ message: "Server Error" });
  }
});
...
router.get("/best-seller", async (req, res) => {
  try {
    ...
    console.error("Error in /best-seller:", error.stack);
    res.status(500).json({ message: "Internal Server Error" });
  }
});

// @route   GET /api/products/new-arrivals
// @desc    Get the latest 13 products
// @access  Public
router.get("/new-arrivals", async (req, res) => {
  try {
    ...
    console.error("Error in /new-arrivals:", error.stack);
    res.status(500).json({ message: "Internal Server Error" });
  }
});

// ⚠️ Keep this LAST to prevent route conflicts
